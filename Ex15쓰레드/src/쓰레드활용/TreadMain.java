...
public class TreadMain {
	...
	public static void main(String[] args) {
		...
		//종료가 안됐어도 다른작업 수행가능
		System.out.println("===멀티쓰레드 : Tread 상속===");
		for(int i=0; i<key.length; i++) {
			Ari a = new Ari(key[i]);
			//실제클래스 안에는 run()에 실제 쓰레드가 동작할 코드 작성 (한가지 작업)
			//start() 쓰레드가 여러개 동시에 동작할 수 있도록 하는 메서드(쓰레드 클래스에 있음)
		...
		for(int i=0; i<key.length; i++) {
			//Runnable 을 구현한 객체를 사용해 Thread 객체 생성
